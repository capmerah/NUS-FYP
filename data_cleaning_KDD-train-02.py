...
def clean_kdd_train(input_file, output_file, label_column='label', drop_columns=['difficulty'], k_features=20):
    """
    Cleans KDDTrain+ data without affecting the label column.
    ...
    6. Retrieves the original names for the selected features.
    7. Label-encodes the target column.
    ...
    print("Initial shape (KDDTrain+):", df.shape)
    ...
    # Determine feature columns (all columns except the label)
    ...
    # Retrieve original names for the selected features using the boolean mask
    ...
    # Combine selected features with the encoded label and save
    final_df = pd.concat([X_selected_df, y_encoded_df], axis=1)
    final_df.to_csv(output_file, index=False)
    print(f"Cleaned KDDTrain+ saved to {output_file}")
    return final_df
...
if __name__ == "__main__":
    ...
    train_output = r"C:\Users\wafi\OneDrive - National University of Singapore\2025-2026 FYP\Test Environment Final\cleaned_KDDTrain-02.csv"
    clean_kdd_train(train_input, train_output)
